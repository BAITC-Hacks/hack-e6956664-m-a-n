...
const server = http.createServer(async (req,res)=>{
  try {
    ...
    if (req.method==='GET' && url.pathname==='/api/health') return sendJson(res,200,{status:'ok',profiles:catalog.length,dataset:'loaded',ai:embeddings.available?'configured':'fallback'});
...
server.listen(PORT,'0.0.0.0',()=>console.log(`HackAlem AI: http://localhost:${PORT} (${catalog.length} профилей)`));
